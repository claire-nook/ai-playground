...
// Generate deterministic rows so pagination and record relocation are observable without a real backend.
function buildRows() {
  const rows = [];
  for (let index = 1; index <= 760; index += 1) {
    ...
    rows.push({
      ...
      referenceText: `${category}-REGION / Category ${category} Regional Reference`,
      description: `這是一筆 Synthetic Business Object 的長文字欄位，用來驗證 General Detail Pattern，而不是模仿任何正式資料表。\n\n長文字不應被硬塞進普通三分之一欄位。Platform 應讓 Feature 用語意化的 presentation hint 表達「這個內容需要完整閱讀寬度」，再由 Responsive Layout 決定實際呈現。\n\n這一段也故意保留多個段落與換行，用來觀察純文字 Long Text 在桌面、iPad 與手機上的閱讀效果。它仍然只是 Plain Text，不因為字很多就偷偷升級成 Rich Text 或 CMS。`,
...
// Presentation span is semantic metadata, not a raw CSS grid instruction exposed to Feature specifications.
function field(label, value, typeLabel = "", presentation = "normal") {
  const allowed = new Set(["normal", "wide", "full"]);
  const span = allowed.has(presentation) ? presentation : "normal";
  return `<div class="detail-field field-${span}"><dt>${escapeHtml(label)}${typeLabel ? `<small>${escapeHtml(typeLabel)}</small>` : ""}</dt><dd>${escapeHtml(displayValue(value))}</dd></div>`;
}
...
function openDetail(recordId) {
  ...
  el("business-fields").innerHTML = [
    ...
    field("狀態", statusMeta[row.status]?.label ?? row.status, "Status"),
    field("Code + Description", row.referenceText, "Reference", "wide"),
    field("金額", formatCurrency(row.amount), "Currency"),
    field("比例", formatPercent(row.ratio), "Percentage"),
    field("日期", row.effectiveDate, "Date"),
    field("日期時間", row.eventAt, "Datetime", "wide"),
