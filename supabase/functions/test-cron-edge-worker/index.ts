    ...
    return jsonResponse(
      {
        ...
        message: "Server-side apikey header is required.",
      },
      401,
    );
  }

  // Gateway JWT verification is disabled for this service-to-service route because
  // current Supabase Secret Keys are not JWTs. The function owns the authorization
  // boundary and admits only the server-side key that Cron B will send as `apikey`.
  if (apiKey !== secretKey) {
    return jsonResponse(
      {
        error: "forbidden",
